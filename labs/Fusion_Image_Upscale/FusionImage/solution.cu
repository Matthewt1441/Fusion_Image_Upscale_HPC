...
//argv[1]
        ...
        dim3 BiCubic_Block(block_dim*scale, block_dim*scale);
        ...
        for (int i = 0; i < 5; i ++)
        {
            ...
            cudaDeviceSynchronize();
            RGB2GreyscaleKernel <<< Grid_16, Block_16 >> >  (d_naive_img_BIC, d_naive_gray_BIC, big_width, big_height);
            cudaDeviceSynchronize();
